...
async function main() {
  ...
  const year1 = await prisma.academicYears.create({
    data: {
      id: 2023, // Manual ID as per original SQL (if not autoincrement) - Adjusted to use autoincrement based on latest schema
  ...
  const year2 = await prisma.academicYears.create({
    data: {
      // id: 2024, // Will be auto-generated
  ...
  const dept2 = await prisma.departments.create({
    data: { school_id: school1.id, name: "Electrical Engineering" },
  });
  const dept3 = await prisma.departments.create({
    data: { school_id: school2.id, name: "Accounting" },
  });
  console.log(
